...
# Author: Junfei Zhou <jeffreychou@my.swjtu.edu.cn>
    ...
    for i, batch_data in enumerate(tqdm(data_loader, desc=infer_info, unit="batch", ascii=True)):
        ...
        with torch.no_grad():
            ...
            if opt.fusion_method == 'late':
                infer_result = inference_utils_v2xreal.inference_late_fusion(batch_data,
                                                        model,
                                                        opencood_dataset)
            elif opt.fusion_method == 'early':
                infer_result = inference_utils_v2xreal.inference_early_fusion(batch_data,
                                                        model,
                                                        opencood_dataset)
                ...
                infer_result = inference_utils_v2xreal.inference_no_fusion_w_uncertainty(batch_data,
                                                                model,
                                                                opencood_dataset)
            ...
            if opt.save_npy:
                ...
                inference_utils_v2xreal.save_prediction_gt(pred_box_tensor,
                                                gt_box_tensor,
                                                batch_data['ego'][
                                                    'origin_lidar'][0],
                                                i,
