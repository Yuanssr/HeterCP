...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def generate_label(self, **kwargs):
        ...
        eps = 1e-6
        anchors_d = np.maximum(anchors_d, eps)
        anchors_d_pos = anchors_d[id_pos]

        gt_dims = np.maximum(gt_box_center[id_pos_gt, 3:6], eps)
        anc_dims = np.maximum(anchors[id_pos, 3:6], eps)
        idx = np.array(index_z)

        targets[index_x, index_y, idx * 7 + 0] = (gt_box_center[id_pos_gt, 0] - anchors[id_pos, 0]) / anchors_d_pos
        targets[index_x, index_y, idx * 7 + 1] = (gt_box_center[id_pos_gt, 1] - anchors[id_pos, 1]) / anchors_d_pos
        targets[index_x, index_y, idx * 7 + 2] = (gt_box_center[id_pos_gt, 2] - anchors[id_pos, 2]) / anc_dims[:, 0]
        targets[index_x, index_y, idx * 7 + 3] = np.log(gt_dims[:, 0] / anc_dims[:, 0])
        targets[index_x, index_y, idx * 7 + 4] = np.log(gt_dims[:, 1] / anc_dims[:, 1])
        targets[index_x, index_y, idx * 7 + 5] = np.log(gt_dims[:, 2] / anc_dims[:, 2])
        targets[index_x, index_y, idx * 7 + 6] = (gt_box_center[id_pos_gt, 6] - anchors[id_pos, 6])
    ...
    def post_process_local(self, data_dict, output_dict):
        ...
        for cav_id in output_dict.keys():
            ...
            if cav_id == 'ego':
                ...
                mask = \
                    torch.gt(prob, self.params['target_args']['non_ego_confidence_threshold'])
            mask = mask.view(1, -1)
            mask_reg = mask.unsqueeze(2).repeat(1, 1, 7)
            ...
            # 对非 ego 的分数做抑制
            if cav_id != 'ego' and len(boxes3d) != 0:
                suppress_factor = self.params['target_args']['non_ego_suppress']
                scores = scores * suppress_factor
    ...
    def post_process_vanilla(self, data_dict, output_dict):
        """
        Process the outputs of the model to 2D/3D bounding box.
        Step1: convert each cav's output to bounding box format
        Step2: project the bounding boxes to ego space.
