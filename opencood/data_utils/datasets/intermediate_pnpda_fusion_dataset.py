...
def getIntermediatepnpdaFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class IntermediateheterFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            self.lidar_modality_name_list = []
            self.camera_modality_name_list = []
            if 'm1' in self.modality_name_list:
                self.lidar_modality_name_list.append('m1')
                self.sensor_type_list.append('lidar')
            if 'm2' in self.modality_name_list:
                self.camera_modality_name_list.append('m2')
                self.sensor_type_list.append('camera')
            if 'm3' in self.modality_name_list:
                self.lidar_modality_name_list.append('m3')
                self.sensor_type_list.append('lidar') if 'lidar' not in self.sensor_type_list else None
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            # lidar
            if "lidar" in self.sensor_type_list or self.visualize:
                # process lidar
                ...
                for modality_name in self.lidar_modality_name_list:
            ...
            label_dict = self.post_processor.generate_pos_region_ranges(
                gt_box_center=object_bbx_center[object_bbx_mask == 1], label_dict=label_dict
            )
            ...
            # camera
            if "camera" in self.sensor_type_list:
                ...
                for modality_name in self.camera_modality_name_list:
                    selected_cav_processed.update(
                        {
        ...
        def __getitem__(self, idx):
            ...
            label_dict = \
                self.post_processor.generate_label(
                    ...
                    mask=mask)
            label_dict = \
                self.post_processor.generate_pos_region_ranges(
                    gt_box_center=object_bbx_center[mask == 1],
        ...
        def collate_batch_train(self, batch):
            ...
            label_torch_dict = \
                self.post_processor.collate_batch_pnpda(label_dict_list)
