...
def getLateheterparallelFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class LateheterparallelFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            # whether to stack same-modality agents together for one-shot inference
            self.group_modality_inputs = params['heter'].get('group_modality_inputs', False)
        ...
        def collate_batch_test(self, batch):
            ...
            # optional holders for grouping same-modality agents
            grouped_feature_lists = {m: [] for m in self.modality_name_list} if self.group_modality_inputs else None
            grouped_cav_ids = {m: [] for m in self.modality_name_list} if self.group_modality_inputs else None
            ...
            for cav_id, cav_content in batch.items():
                ...
                if sensor_type == "lidar":
                    ...
                    if self.group_modality_inputs:
                        grouped_feature_lists[modality_name].append(cav_content[f'processed_features_{modality_name}'])
                        grouped_cav_ids[modality_name].append(cav_id)
                ...
                if sensor_type == 'camera':
                    ...
                    if self.group_modality_inputs:
                        grouped_feature_lists[modality_name].append(cav_content[f'image_inputs_{modality_name}'])
                        grouped_cav_ids[modality_name].append(cav_id)
            ...
            if self.group_modality_inputs:
                grouped_inputs = {}
                for modality_name in self.modality_name_list:
                    if len(grouped_feature_lists[modality_name]) == 0:
                        continue
                    sensor_type = self.sensor_type_dict[modality_name]
                    if sensor_type == "lidar":
                        merged_feature_dict = merge_features_to_dict(grouped_feature_lists[modality_name])
                        processed_group = eval(f"self.pre_processor_{modality_name}").collate_batch(merged_feature_dict)
                    else:  # camera
                        processed_group = merge_features_to_dict(grouped_feature_lists[modality_name], merge='stack')
                    grouped_inputs.update({f"inputs_{modality_name}": processed_group})
                    grouped_inputs.update({f"cav_ids_{modality_name}": grouped_cav_ids[modality_name]})
                
                output_dict['ego']['grouped_inputs'] = grouped_inputs
                output_dict['ego']['grouped_cav_id_list'] = list(batch.keys())
                output_dict['ego']['grouped_inputs']['agent_modality_list'] = [batch[cav]['modality_name'] for cav in batch.keys()]
    ...
    return LateheterparallelFusionDataset