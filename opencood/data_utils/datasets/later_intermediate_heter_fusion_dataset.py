...
def getLateintermediateheterFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class LateintermediateheterFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            if params['fusion']['dataset'] == 'v2xreal':
            ...
            for modality_name, modal_setting in params['heter']['modality_setting'].items():
                ...
                elif modal_setting['sensor_type'] == 'lidar_camera':
                    setattr(self, f"pre_processor_{modality_name}", build_preprocessor(modal_setting['lidar']['preprocess'], train))
                    setattr(self, f"data_aug_conf_{modality_name}", modal_setting['camera']['data_aug_conf'])
                else:
        ...
        def __getitem__(self, idx):
            ...
            input_dict = {m: [] for m in self.modality_name_list}
            agent_modality_list = []
            object_stack = []
            object_id_stack = []
            single_label_list = []
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                if sensor_type == "lidar":
                    input_dict[modality_name].append(selected_cav_processed[f"processed_features_{modality_name}"])
                elif sensor_type == "camera":
                    input_dict[modality_name].append(selected_cav_processed[f"image_inputs_{modality_name}"])
                elif sensor_type == "lidar_camera":
                    input_dict[modality_name].append(selected_cav_processed[f"processed_features_{modality_name}"])
                    input_dict[modality_name].append(selected_cav_processed[f"image_inputs_{modality_name}"])
                else:
                    raise
                ...
                if self.visualize or self.kd_flag:
                    ...
                    if sensor_type == "lidar" and self.kd_flag:
                        input_proj_dict[modality_name].append(selected_cav_processed[f"processed_features_{modality_name}_proj"])
                    if sensor_type == "lidar_camera" and self.kd_flag:
                        input_proj_dict[modality_name].append(selected_cav_processed[f"processed_features_{modality_name}_proj"])    
            ...
            if self.params['fusion']['dataset'] == 'dairv2x':
                ...
                # exclude all repetitive objects, OPV2V-H, V2XSet, V2X-Real
                unique_indices = \
            ...
            for modality_name in self.modality_name_list:
                if self.sensor_type_dict[modality_name] == "lidar":
                    ...
                    merged_image_inputs_dict = merge_features_to_dict(input_dict[modality_name], merge='stack')
                    processed_data_dict['ego'].update({f'input_{modality_name}': merged_image_inputs_dict}) # maybe None
                elif self.sensor_type_dict[modality_name] == "lidar_camera":
                    merged_feature_dict = merge_features_to_dict(input_dict[modality_name], merge='stack') 
                    processed_data_dict['ego'].update({f'input_{modality_name}': merged_feature_dict}) # maybe None
            if self.kd_flag:
                ...
                for modality_name in self.modality_name_list:
                    processed_data_dict['ego'].update({
                        f'input_{modality_name}_proj': merge_features_to_dict(input_proj_dict[modality_name]) # maybe None
                        })
            ...
            processed_data_dict['ego'].update(
                ...
                'num_anchors_per_location': self.num_anchors_per_location \
                    if hasattr(self, 'num_anchors_per_location') else None,  # only used in v2xreal
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            # lidar
            if sensor_type in ("lidar", "lidar_camera") or self.visualize:
                # process lidar
                ...
                if sensor_type in ("lidar", "lidar_camera"):
            ...
            # camera
            if sensor_type in ("camera", "lidar_camera"):
        ...
        def collate_batch_train(self, batch):
            ...
            for i in range(len(batch)):
                ...
                ego_id_list.append(ego_dict['ego_id'])

                for modality_name in self.modality_name_list:
                    if ego_dict[f'input_{modality_name}'] is not None:
                        inputs_dict[modality_name].append(ego_dict[f'input_{modality_name}']) # OrderedDict() if empty?
                ...
                # 收集该样本的非 ego 车辆 T 矩阵（字典形式）
                non_ego_data_dict = {}
                for cav_id, cav_content in batch[i].items():
                    if cav_id != 'ego' and 'transformation_matrix' in cav_content:
                        non_ego_data_dict[cav_id] = {
                            'transformation_matrix_clean': torch.from_numpy(
                ...
                non_ego_data_list.append(non_ego_data_dict)
            ...
            for modality_name in self.modality_name_list:
                if len(inputs_dict[modality_name]) != 0:
                    if self.sensor_type_dict[modality_name] == "lidar":
                        ...
                        merged_image_inputs_dict = merge_features_to_dict(inputs_dict[modality_name], merge='cat')
                        output_dict['ego'].update({f'inputs_{modality_name}': merged_image_inputs_dict})
                    elif self.sensor_type_dict[modality_name] == "lidar_camera":
                        merged_feature_dict = merge_features_to_dict(inputs_dict[modality_name], merge='cat') 
                        output_dict['ego'].update({f'inputs_{modality_name}': merged_feature_dict}) # maybe None
            ...
            output_dict['ego'].update({'object_bbx_center': object_bbx_center,
                                    ...
                                    'anchor_box': self.anchor_box \
                                        if self.params['fusion']['dataset'] == 'v2xreal' \
                                        else self.anchor_box_torch}) # if v2xreal case, anchor_box is a list

            output_dict['ego'].update({'non_ego_data_list': non_ego_data_list})
            ...
            output_dict['ego'].update({'cav_id_list': cav_id_list})
            output_dict['ego'].update({'ego_id_list': ego_id_list})
            ...
            if self.kd_flag:
                ...
                for modality_name in self.modality_name_list:
                    if len(inputs_proj_dict[modality_name]) != 0 and self.sensor_type_dict[modality_name] == "lidar":
        ...
        def post_process_local(self, data_dict, output_dict):
            """
            Process the outputs of the model to 2D/3D bounding box.
            ...
            if self.params['fusion']['dataset'] == 'v2xreal':
                ...
                pred_box_tensor, pred_score = \
                        self.post_processor.post_process_local(data_dict, output_dict)
                gt_box_tensor = self.post_processor.generate_gt_bbx(data_dict)

                return pred_box_tensor, pred_score, gt_box_tensor

    return LateintermediateheterFusionDataset
