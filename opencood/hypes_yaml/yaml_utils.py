...
def load_yaml(file, opt=None):
    ...
    if "yaml_parsers" in param:
        parser_map = param.pop("yaml_parsers")
        modality_setting = param.get("heter", {}).get("modality_setting", {})
        for modal, parser_name in parser_map.items():
            if modal not in modality_setting:
                continue
            parser_func = globals()[parser_name]
            modality_setting[modal] = parser_func(modality_setting[modal])
    return param
