...
class DDPCheckMixin:
    """
    异构模型多卡训练时不同batch中可能缺失某些模态, 导致部分参数未被使用, 进而引发DDP死锁。
    该类用于自动处理 DDP 中未使用的参数，防止死锁。
    """
    def _register_ddp_hooks(self):
        self._executed_modules = set()
        
        def get_hook(name):
            def hook(module, input, output):
                self._executed_modules.add(name)
            return hook

        # 自动为所有以 encoder_, backbone_, shrinker_, message_extractor_ 开头的模块注册 hook
        for name, module in self.named_children():
            if any(name.startswith(prefix) for prefix in ['encoder_', 'backbone_', 'shrinker_', 'message_extractor_']):
                module.register_forward_hook(get_hook(name))

    def _handle_unused_parameters(self):
        """
        在 forward 结束前调用，对未执行的模块生成伪梯度
        """
        dummy_loss = 0.0
        # 遍历所有需要检查的子模块
        for name, module in self.named_children():
            if any(name.startswith(prefix) for prefix in ['encoder_', 'backbone_', 'shrinker_', 'message_extractor_']):
                if name not in self._executed_modules:
                    # 如果该模块本次 forward 没被执行
                    for p in module.parameters():
                        if p.requires_grad:
                            dummy_loss += p.sum() * 0.0
        
        # 清空状态，为下一次 forward 做准备
        self._executed_modules.clear()
        return dummy_loss

class HeterModelBaselineWGenCommStage2(nn.Module, DDPCheckMixin):
    def __init__(self, args):
        super(HeterModelBaselineWGenCommStage2, self).__init__()
        self.args = args
    ...
    def forward(self, data_dict):
        ...
        # Handle unused parameters for DDP
        if self.training:
            dummy_loss = self._handle_unused_parameters()
            cls_preds = cls_preds + dummy_loss
...
class HeterModelBaselineWGenCommStage2_origin(nn.Module):
    def __init__(self, args):
        super(HeterModelBaselineWGenCommStage2_origin, self).__init__()
        self.args = args
        self.missing_message = args['missing_message'] if 'missing_message' in args else False
        self.gencomm = GenComm(args['gencomm'])
        modality_name_list = list(args.keys())
