...
class HeterModelBaselineWB2F(nn.Module):
    def __init__(self, args):
        super(HeterModelBaselineWB2F, self).__init__()
        self.args = args
    ...
    def forward(self, data_dict):
        ...
        for scene_idx, (cav_ids, flags, non_ego_feats) in enumerate(
            zip(data_dict['cav_id_list'], ego_flag_list, non_ego_feature_2d_list)
        ):
            non_ego_ptr = 0
            for cid, is_ego in zip(cav_ids, flags):
                if is_ego:
                    ...
                    recon_feat = non_ego_feats[non_ego_ptr]
