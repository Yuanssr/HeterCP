...
import os
from opencood.models.gencomm_modules.unet import DiffusionUNet
...
class DiffusionBEVGen(nn.Module):
    ...
    def forward(self, x0, cond):
        ...
        if self.training:
            ...
            for b_idx in range(bs):
                x = cond[b_idx:b_idx+1]
                x_start = x0[b_idx:b_idx+1]
            ...
            noisy_masks = {}
            x = cond
            x_start = x0 * self.signal_scaling_rate #batch['mask_'+task]
            noise = default(None, lambda: torch.randn_like(x_start))
            t = torch.ones(x.shape[0], device=x.device).long() * (self.num_timesteps - 1)
            ego_feat = x0[0].unsqueeze(0)
            x_start_noisy_t1 = self.q_sample(x_start=ego_feat, t=torch.tensor([1], device=x.device), noise=torch.randn_like(ego_feat))
            x_start_noise_t2 = self.q_sample(x_start=ego_feat, t=torch.tensor([2], device=x.device), noise=torch.randn_like(ego_feat))
            _x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
            noisy_masks= _x_noisy
            noisy_masks = self.p_sample_loop(x, noisy_masks, x.shape)
            all_noisy_masks = noisy_masks
            data_dict['t1'] = x_start_noisy_t1
            data_dict['t2'] = x_start_noise_t2
