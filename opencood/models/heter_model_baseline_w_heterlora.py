...
def build_heter_lora_model(args):
    """
    返回注入 LoRA 的模型实例；若未提供 args['lora']，则返回原模型。
    """
    base = HeterModelBaselineWHeterLoRa(args)
    lora_args = args.get("lora")
    if not lora_args:
        return base
    lora_cfg = LoraConfig(
        r=lora_args.get("r", 8),
        lora_alpha=lora_args.get("alpha", 16),
        lora_dropout=lora_args.get("dropout", 0.05),
        target_modules=lora_args.get("target_modules", ["conv", "fusion_net", "q", "k", "v"]),
        bias="none",
        task_type="FEATURE_EXTRACTION",
    )
    return get_peft_model(base, lora_cfg)

class HeterModelBaselineWHeterLoRa(nn.Module):
    def __init__(self, args):
        super(HeterModelBaselineWHeterLoRa, self).__init__()
        self.args = args
