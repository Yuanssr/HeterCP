...
class HeterModelFusionBaseline(nn.Module):
    def __init__(self, args):
        super(HeterModelFusionBaseline, self).__init__()
        self.args = args
        modality_name_list = list(args.keys())
        modality_name_list = [x for x in modality_name_list if x.startswith("m") and x[1:].isdigit()] 
        self.modality_name_list = modality_name_list
        self.fix_modules = ['cls_head', 'reg_head', 'dir_head']
        ...
        if 'compressor' in args:
            ...
            self.compress_train_init()

        self.model_train_init()
        # check again which module is not fixed.
        check_trainable_module(self)

    def compress_train_init(self):
        if self.compress:
            # freeze all
            self.eval()
