...
class HeterModelBaseline(nn.Module):
    def __init__(self, args):
        ...
        for modality_name in self.modality_name_list:
            ...
            #mutil modality for single vehicle
            if sensor_name == "lidar_camera":
                """
                Mutil Modality Encoder building
                """
                setattr(self, f"encoder_{modality_name}", encoder_class(model_setting['encoder_args']))
                if model_setting['encoder_args']['camera'].get("depth_supervision", False):
                    setattr(self, f"depth_supervision_{modality_name}", True)
                else:
                    setattr(self, f"depth_supervision_{modality_name}", False)

            #single modality for single vehicle 
            else:
                """
                Encoder building
                """
