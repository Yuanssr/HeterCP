...
from opencood.models.sub_modules.box2feature_generator import Box2FeatureGenerator, Box2FeatureGeneratorV2
...
class HeterModelBaselineWB2FDiff2(nn.Module):
    """
    改动:
    box2featuregenerator:
    用旋转矩形/凸多边形栅格化，而不是轴对齐矩形
    对每个栅格做累加并除以计数，避免重叠覆盖
    ego的检测头解冻
    """
    def __init__(self, args):
        super(HeterModelBaselineWB2FDiff2, self).__init__()
        self.args = args
        ...
        setattr(self, f"cls_head_ego",nn.Conv2d(args['in_head'], args['anchor_number'] * self.num_class * self.num_class,
                                  kernel_size=1))
        setattr(self, f"reg_head_ego", nn.Conv2d(args['in_head'], 7 * args['anchor_number'] * self.num_class,
                                    kernel_size=1))
        setattr(self, f"dir_head_ego", nn.Conv2d(args['in_head'], args['dir_args']['num_bins'] * args['anchor_number'],
                                    kernel_size=1)) # BIN_NUM = 2

        """
        box to feature generator building
        """
        setattr(self, f"box2featuregenerator", Box2FeatureGeneratorV2(model_setting['box2feature']))
        ...
        self.perception_loss = args.get('perception_loss', False)
    ...
    def forward(self, data_dict):
        ...
        if self.supervise_single:
            ...
            output_dict.update({'cls_preds_single': cls_preds_before_fusion,
                                'reg_preds_single': reg_preds_before_fusion,
                                'dir_preds_single': dir_preds_before_fusion})
        if self.perception_loss:
            cls_preds_before_fusion = getattr(self, f"cls_head_{ego_mod}")(heter_feature_2d_new)
            reg_preds_before_fusion = getattr(self, f"reg_head_{ego_mod}")(heter_feature_2d_new)
            dir_preds_before_fusion = getattr(self, f"dir_head_{ego_mod}")(heter_feature_2d_new)
            output_dict.update({'cls_preds_single': cls_preds_before_fusion,
                                'reg_preds_single': reg_preds_before_fusion,
                                'dir_preds_single': dir_preds_before_fusion})
        """
        ...
        cls_preds = getattr(self, f"cls_head_ego")(fused_feature)
        reg_preds = getattr(self, f"reg_head_ego")(fused_feature)
        dir_preds = getattr(self, f"dir_head_ego")(fused_feature)
