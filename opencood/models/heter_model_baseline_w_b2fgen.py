...
from opencood.models.sub_modules.diffusion_bev import DiffusionBEV
...
class HeterModelBaselineWB2FGen(nn.Module):
    def __init__(self, args):
        super(HeterModelBaselineWB2FGen, self).__init__()
        self.args = args
        ...
        self.diffusion = DiffusionBEV({
            "x_channels": self.C_target,
            "cond_channels": self.C_target if self.cond_align is None else self.C_target,  # 对齐后通道
            "base_channels": args.get("diff_base_channels", 128),
            "time_embed_dim": args.get("diff_time_dim", 256),
            "timesteps": args.get("diff_timesteps", 1000),
            "beta_start": args.get("diff_beta_start", 1e-4),
            "beta_end": args.get("diff_beta_end", 2e-2),
})
        ...
        # 确保 box2featuregenerator 可训练
        self.unfreeze_box2featuregenerator()
    ...
    def unfreeze_box2featuregenerator(self):
        """
        Make sure box2featuregenerator remains trainable.
        """
        if hasattr(self, "box2featuregenerator"):
            for p in self.box2featuregenerator.parameters():
    ...
    def forward(self, data_dict):
        ...
        flat_is_ego = [flag for scene in ego_flag_list for flag in scene]
        orig_non_ego = {cid: feat for cid, is_ego, feat in zip(flat_cav_ids, flat_is_ego, heter_feature_2d_list) if not is_ego}
        ...
        x0 = torch.stack(x0_list)            # [N_agents, C, H, W]
        gt_feature = torch.stack(heter_feature_2d_list)  # 原始各车特征，供 loss 对齐

        cond_list = []
        for scene_idx, (cav_ids, flags, non_ego_feats) in enumerate(zip(data_dict['cav_id_list'], ego_flag_list, non_ego_feature_2d_list)):
            non_ego_ptr = 0
            for is_ego in flags:
                if is_ego:
                    cond_list.append(torch.zeros_like(ego_feature[scene_idx]))
                else:
                    cond_list.append(non_ego_feats[non_ego_ptr])
                    non_ego_ptr += 1
        cond = torch.stack(cond_list)        # [N_agents, C, H, W]
        if self.cond_align is not None:
            cond = self.cond_align(cond)

        if self.training:
            loss_diff, x0_hat = self.diffusion(x0, cond)
            output_dict['loss_diff'] = loss_diff
        else:
            with torch.no_grad():
                if data_dict.get("eval_with_recon_loss", False):
                    loss_diff, x0_hat = self.diffusion(x0, cond)  # 有 loss 值但不反传
                    output_dict['loss_diff'] = loss_diff
                else:
                    x0_hat = self.diffusion.sample(cond, steps=20)  # 只采样，不算 loss
        ...
        for scene_idx, (cav_ids, flags, non_ego_feats) in enumerate(zip(data_dict['cav_id_list'], ego_flag_list, non_ego_feature_2d_list)):
            for cid, is_ego in zip(cav_ids, flags):
                if is_ego:
                    ...
                    recon_feat = x0_hat[ptr]
                    heter_feature_2d_list_new.append(recon_feat)
                    reconstruction_pairs.append((recon_feat, orig_non_ego[cid]))
                    ptr += 1
        
        
        output_dict['reconstruction_items'] = reconstruction_pairs
