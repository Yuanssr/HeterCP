...
class BEVFusion(nn.Module):
    def __init__(self, args):
        ...
        if args.get("camera") is not None:
            self.encoders["camera"] = nn.ModuleDict(
                {
                    "backbone": build_backbone(args["camera"]["backbone"]),
                    "neck": build_neck(args["camera"]["neck"]),
                    "vtransform": build_vtransform(args["camera"]["vtransform"]),
                }
            )
        if args.get("lidar") is not None:
            if args["lidar"]["voxelize"].get("max_num_points", -1) > 0:
                voxelize_module = Voxelization(**args["lidar"]["voxelize"])
            else:
                voxelize_module = DynamicScatter(**args["lidar"]["voxelize"])
            self.encoders["lidar"] = nn.ModuleDict(
                {
                    "voxelize": voxelize_module,
                    "backbone": build_backbone(args["lidar"]["backbone"]),
                }
            )
            self.voxelize_reduce = args["lidar"].get("voxelize_reduce", True)


        if args.get("fuser") is not None:
            self.fuser = build_fuser(args["fuser"])
        else:
            self.fuser = None
    ...
    def extract_camera_features(self, data_dict, modality_name):
        image_inputs_dict = data_dict[f'inputs_{modality_name}']  # B x N x C x H x W
    ...
    def forward(
        ...
        for sensor in (
            ...
            if sensor == "camera":
                feature = self.extract_camera_features(data_dict, modality_name)
                
            elif sensor == "lidar":
                feature = self.extract_features(data_dict, sensor)
            else:
