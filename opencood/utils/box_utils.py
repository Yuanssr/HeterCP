...
def mask_boxes_outside_range_numpy_v2xreal(boxes, limit_range, order,
                                   min_num_corners=2, return_mask=False):
    """
    Parameters
    ----------
    boxes: np.ndarray
        (N, 8) [x, y, z, dx, dy, dz, heading, class], (x, y, z) is the box center
    ...
    new_boxes = boxes.copy()
    if (boxes.shape[1] == 8 or boxes.shape[1] == 7 ) and len(boxes.shape) == 2:
        new_boxes = boxes_to_corners_3d(new_boxes, order)
...
def project_world_objects_v2xreal(object_dict,
                          output_dict,
                          lidar_pose,
                          lidar_range,
                          order,
    ...
    for object_id, object_content in object_dict.items():
        ...
        bbx_lidar = np.expand_dims(bbx_lidar[:, :3], 0)
        bbx_lidar = corner_to_center(bbx_lidar, order=order)
        bbx_lidar, mask = mask_boxes_outside_range_numpy_v2xreal(bbx_lidar,
                                                   lidar_range,
                                                   order,
                                                   return_mask=True)
        obj_type = obj_type[mask]

        if bbx_lidar.shape[0] > 0:
            output_dict.update({object_id: np.concatenate([bbx_lidar, obj_type[None, :]], axis=-1)})

def project_world_objects_v2xreal_stamp(object_dict,
                          output_dict,
                          lidar_pose,
                          lidar_range,
                          order,
...
def project_world_objects_stamp(object_dict,
                          output_dict,
                          lidar_pose,
                          lidar_range,
                          order,
...
def nms_rotated_mmcv(boxes, scores, threshold, top_k=1000):
    """
    使用 mmcv.ops.nms_rotated 的 GPU 版旋转 NMS。
    boxes: (N, 4, 2) 角点格式
    scores: (N,)
    threshold: IoU 阈值
    top_k: 仅保留最高分前 top_k 做 NMS，降低开销
    返回 numpy.int32 索引
    """
    if boxes.numel() == 0:
        return np.array([], dtype=np.int32)
    ...
    if scores.numel() > top_k:
        scores_sel, idx_sel = scores.topk(top_k)
        boxes_sel = boxes[idx_sel]
    else:
        scores_sel, idx_sel = scores, None
        boxes_sel = boxes

    # 角点 -> (cx, cy, w, h, angle[radians])
    v01 = boxes_sel[:, 1] - boxes_sel[:, 0]
    v03 = boxes_sel[:, 3] - boxes_sel[:, 0]
    w = torch.linalg.norm(v01, dim=1)
    h = torch.linalg.norm(v03, dim=1)
    angle = torch.atan2(v01[:, 1], v01[:, 0])  # 弧度
    cxcy = boxes_sel.mean(dim=1)
    rboxes = torch.stack([cxcy[:, 0], cxcy[:, 1], w, h, angle], dim=1)  # (N,5)

    # mmcv 返回 (dets, keep)，keep 是索引
    _, keep = mmcv_nms_rotated(rboxes, scores_sel, threshold)

    if idx_sel is not None:
        keep = idx_sel[keep]

    return keep.detach().cpu().numpy().astype(np.int32)
...
def project_world_visible_objects_stamp(object_dict,
                          output_dict,
                          lidar_pose,
                          lidar_range,
                          order,
