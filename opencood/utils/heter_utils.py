...
class Adaptor:
    ...
    def reassign_cav_modality_diffcav(self, modality_name, idx_in_cav_list):
        """
        work in basedataset -> reinitialize
        """
        # this function makes sure the cav modality is different from the ego modality
        # always assign the ego_modality to idx 0 in cav_list
        if idx_in_cav_list == 0:
            return np.random.choice(self.ego_modality.split("&"))
        self.cav_preferece = {k: v for k, v in self.cav_preferece.items() if k not in self.ego_modality.split("&")}
        return random.choices(list(self.cav_preferece.keys()), weights=self.cav_preferece.values())[0]
