...
def eval_final_results(result_stat, save_path, global_sort_detections, infer_info=None):
    dump_dict = {}
    ...
    output_file = 'eval' if not global_sort_detections else 'eval_global_sort'
    if infer_info is None:
        yaml_utils.save_yaml(dump_dict, os.path.join(save_path, f'{output_file}.yaml'))
    else:
        yaml_utils.save_yaml(dump_dict, os.path.join(save_path, f'{output_file}_{infer_info}.yaml'))
