...
class PointPillarMPDALoss(nn.Module):
    ...
    def forward(self, output_dict, target_dict, suffix="", validate=False):
        """
        Parameters
        ----------
        output_dict : dict
        ...
        if self.da and not validate:
            ...
            da_targets = to_device(torch.zeros_like(da_feature,
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix="",iter=None):
        ...
        print("[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"
              " || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || Da Loss: %.4f" % (
                  epoch, batch_id + 1, batch_len, suffix,
                  total_loss, cls_loss, reg_loss, dir_loss, iou_loss, da_loss))
        ...
        wandb.log({ 'Loss'+ suffix: total_loss,
                    ...
                    'Da_loss'+suffix: da_loss}, step=iter)
        """
