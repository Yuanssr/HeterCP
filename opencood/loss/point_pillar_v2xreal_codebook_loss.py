...
class PointPillarV2XRealCodebookLoss(nn.Module):
    def __init__(self, args):
        super(PointPillarV2XRealCodebookLoss, self).__init__()
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None, iter=None):
        ...
        codebook_loss = self.loss_dict['codebook_loss']
        if pbar is None:
            print("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
                " || Loc Loss: %.4f || Codebook Loss: %.4f" % (
                    epoch, batch_id + 1, batch_len,
                    total_loss.item(), conf_loss.item(), reg_loss.item(), codebook_loss.item()))
        else:
