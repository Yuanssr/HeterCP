...
class PointPillarV2XRealMpdaLoss(nn.Module):
    def __init__(self, args):
        super(PointPillarV2XRealMpdaLoss, self).__init__()
    ...
    def forward(self, output_dict, target_dict, validate=False):
        """
        Parameters
        ----------
        output_dict : dict
        target_dict : dict
        """
        rm = output_dict['reg_preds']
        psm = output_dict['cls_preds']
        ...
        da_loss = torch.zeros(1).to(device)

        if self.da and not validate:
            ...
            da_targets = torch.zeros_like(da_feature,
            ...
            da_loss = F.binary_cross_entropy_with_logits(da_feature,
                                                         da_targets)

            total_loss += da_loss
            
            # da_loss = F.binary_cross_entropy_with_logits(domain_logits, domain_labels)
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None, iter=None):
        """
        ...
        da_loss = self.loss_dict['da_loss']
        if pbar is None:
            print("[epoch %d][%d/%d], || Loss: %.4f || Conf Loss: %.4f"
                " || Loc Loss: %.4f || Da Loss: %.4f" % (
                    epoch, batch_id + 1, batch_len,
                    total_loss.item(), conf_loss.item(), reg_loss.item(), da_loss.item()))
        else:
