...
from opencood.loss.point_pillar_depth_diffusion_loss import PointPillarDepthDiffusionLoss
from opencood.loss.point_pillar_loss import PointPillarLoss

class PointPillarDepthDiffusionPerceptionLoss(PointPillarDepthDiffusionLoss):
    def __init__(self, args):
        super().__init__(args)
        self.perception_weight = args.get('perception', {}).get('weight', 1.0)
        self.perception_loss = PointPillarLoss(args)
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        perception_loss = self.perception_loss(output_dict, target_dict['single'], suffix="_single") * self.perception_weight 
        total_loss = total_loss + perception_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix="", iter=None):
        ...
        diffsuion_loss = self.loss_dict.get('diffsuion_loss', 0)
        perception_loss = self.loss_dict.get('perception_loss', 0)

        print("[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"
              " || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || Depth Loss: %.4f || Diffusion Loss: %.4f | Perception Loss: %.4f " % (
                  epoch, batch_id + 1, batch_len, suffix,
                  total_loss, cls_loss, reg_loss, dir_loss, iou_loss, depth_loss, diffsuion_loss, perception_loss))
        ...
        if not writer is None:
            ...
            writer.add_scalar('Depth_loss' + suffix, depth_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Diffusion_loss' + suffix, diffsuion_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Perception_loss' + suffix, perception_loss,
                            epoch*batch_len + batch_id)                
