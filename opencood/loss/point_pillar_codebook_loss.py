...
class PointPillarCodebookLoss(PointPillarLoss):
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix="", iter=None):
        ...
        print("[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"
              " || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || Depth Loss: %.4f || Codebook Loss: %.4f" % (
                  epoch, batch_id + 1, batch_len, suffix,
                  total_loss, cls_loss, reg_loss, dir_loss, iou_loss, depth_loss, codebook_loss))
        ...
        if not writer is None:
            ...
            writer.add_scalar('Depth_loss' + suffix, depth_loss,
                            epoch*batch_len + batch_id)
        #wandb.log({"epoch": epoch, "train_loss_iter": total_loss, "Conf_loss": cls_loss, "Reg_loss": reg_loss, 
                  # "Dir_loss": dir_loss, "Iou_loss": iou_loss, "depth_loss": depth_loss, "codebook_loss": codebook_loss},
              #    step=iter)
