...
class PointPillarDepthDiffusionLoss(PointPillarDepthLoss):
    def __init__(self, args):
        super().__init__(args)
        self.recon_weight = args.get('reconstruction', {}).get('weight', 1.0)
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        diffsuion_loss = self.diff_loss(gt_feature, pred_feature) * self.recon_weight
        total_loss = total_loss + diffsuion_loss
        self.loss_dict.update({'total_loss': total_loss.item(),
                               'diffsuion_loss': diffsuion_loss.item()})
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix="", iter=None):
        ...
        print("[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"
              " || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || Depth Loss: %.4f || Diffusion Loss: %.4f" % (
                  epoch, batch_id + 1, batch_len, suffix,
                  total_loss, cls_loss, reg_loss, dir_loss, iou_loss, depth_loss, diffsuion_loss))
